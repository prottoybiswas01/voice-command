"""
Intent Parser for Motu Assistant (Phase 6 Final Alexa-Grade Upgrade).
...
Pomodoro & Productivity Hub, Security Audit, Smart Music, Media Key Controls (Next, Pause, Resume, Stop),
Smart Home IoT, Vision AI, Local RAG Knowledge Base, Macro Workflows, AI Personalities, System Diagnostics, Plugins, and LLM Chat.
"""
...
class IntentParser:
    ...
    def parse(self, text: str) -> Intent:
        ...
        # 3. Media Key Controls (Next Song, Previous Song, Pause, Resume, Stop)
        if any(w in clean for w in ["next song", "change song", "next track", "গান বদলাও", "পরের গান", "নেক্সট গান", "গান চেঞ্জ করো", "অন্য গান", "পরিবর্তন করে দাও"]):
            return Intent(name="media_control", action_type="media", params={"command": "next"}, raw_prompt=text)
        if any(w in clean for w in ["pause music", "pause song", "pause", "গান থামাও", "পজ করো", "পজ"]):
            return Intent(name="media_control", action_type="media", params={"command": "pause"}, raw_prompt=text)
        if any(w in clean for w in ["resume music", "resume song", "resume", "গান রিজিউম", "আবার চালাও"]):
            return Intent(name="media_control", action_type="media", params={"command": "play"}, raw_prompt=text)
        if any(w in clean for w in ["stop music", "stop song", "stop playing", "গান বন্ধ করো"]):
            return Intent(name="media_control", action_type="media", params={"command": "stop"}, raw_prompt=text)

        # 4. Smart Music Playback (Explicit Music Patterns Only)
        explicit_music = [
            "play music", "play song", "play romantic", "play playlist",
            "গান চালাও", "গান বাজাও", "মিউজিক চালাও", "প্লে মিউজিক", "গান প্লে করো",
            "রোমান্টিক গান", "রোমান্টিক মিউজিক", "গান শোনো", "গান শুনব", "একটি গান", "একটা গান"
        ]
        if any(p in clean for p in explicit_music) or (("play" in clean or "চালাও" in clean or "বাজাও" in clean) and ("music" in clean or "song" in clean or "গান" in clean or "মিউজিক" in clean or "রোমান্টিক" in clean)):
            is_romantic = "রোমান্টিক" in clean or "romantic" in clean
            clean_query = clean
            for term in ["আমার জন্য", "একটি", "একটা", "গান", "মিউজিক", "প্লে করো", "প্লে", "চালাও", "বাজাও", "শুনব", "play", "music", "song", "তো"]:
                clean_query = clean_query.replace(term, "").strip()
            if is_romantic:
                clean_query = "romantic music playlist"
            elif not clean_query:
                clean_query = "top hits music playlist"
            return Intent(name="play_smart_music", action_type="smart_music", params={"query": clean_query}, raw_prompt=text)

        # 5. Phase-6 Macro Workflows & Scenes ("start work mode", "good night")
        ...
        # 6. File System & Folder Creation (Bangla & English)
        ...
        # 7. Document & File Search / RAG Knowledge Search
        ...
        # 8. Phase-6 System Health Diagnostics
        ...
        # 9. Phase-6 AI Personalities & Ollama Model Switching
        ...
        # 11. Phase-4 Smart Home IoT Commands
        ...
        # 12. Phase-3 Win32 Application Window Controls
        ...
        # 18. Playwright Browser Automation
        browser_sites = {
        ...
        # Fallback to LLM Chat (For all general questions, explanations, and arbitrary prompts)
